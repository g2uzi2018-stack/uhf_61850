...
- 产品托管的 Modbus/TCP 与 IEC 61850 服务监听 `0.0.0.0`，因此 Web 修改接口 IP 后无需重绑旧地址；显式命令行监听参数仍只绑定指定地址。
...
DHCP stage 使用板上已确认存在的 `dhclient` 和随发布包安装的 root-owned 固定 hook；hook 只把 OFFER/BOUND 写入事务服务，不运行发行版任意脚本。15 秒内取租失败则原样回滚；成功后 helper 把租约地址作为辅助地址、候选默认路由使用更差 metric，并在页面返回实际地址/租期。confirm 后才通过板级适配器更新 `/etc/net.conf`、`/etc/net2.conf` 中的 DHCP 模式，供 `/etc/htnet/ifconfig-eth0`、`ifconfig-eth1` 使用并接管续租；rollback 终止候选 PID、删除候选地址/路由/lease 并恢复旧文件。续租、NAK、地址变化和无 DHCP 服务器都必须故障注入。
...
板卡启动使用 `/etc/htnet/ifconfig-eth0`、`ifconfig-eth1` 和 `/etc/net.conf`/`net2.conf`。Linux backend 通过板级适配器读写两个 `net*.conf`，保留厂商启动脚本不变；业务代码不得直接依赖旧 `/data/modify_net*.sh`，因为旧脚本会立即重启且缺少服务端校验。
