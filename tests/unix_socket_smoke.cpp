...
int main() {
    const std::filesystem::path path =
        std::filesystem::temp_directory_path() / "uhf-privileged-socket-smoke" / "helper.sock";
    ...
    std::thread worker([&server] { assert(server.run() == 0); });
    ...
    assert(success.ok && success.code == "pong" && success.body == "ready");
    const uhf::privileged::Reply rejected = client.request("exec rm -rf /");
    assert(!rejected.ok && rejected.code == "rejected");
    ...
    assert(!oversized_reply.ok && oversized_reply.code == "message_too_large");
