...
int main() {
    ...
    const std::vector<std::uint8_t> stale_response = server.handle_request(valid);
    if (!expect(stale_response.size() == 15U, "stale response size") ||
        !expect(stale_response[3] == 0xFFU && stale_response[4] == 0xCEU,
